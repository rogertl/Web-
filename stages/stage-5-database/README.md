# 阶段 5：数据层与后端逻辑

> 前置依赖：阶段 4

## 学习目标

能看懂数据库操作代码的位置和安全方式，掌握数据验证和错误处理。

## 概念清单

- [x] 5.0 [数据库基础概念](./00-database-fundamentals.md)（主键、外键、索引、事务、关系类型）
- [x] 5.1 [数据库连接与 server-only](./01-database-connection-server-only.md)
- [x] 5.2 [环境变量与配置](./02-environment-variables-configuration.md)
- [x] 5.3 [Prisma ORM 基础](./03-prisma-orm-basics.md)
- [x] 5.4 [CRUD 操作在 Next.js 中的常见位置](./04-crud-operations-nextjs.md)
- [x] 5.5 [zod 数据验证](./05-zod-data-validation.md)
- [x] 5.6 [错误处理与安全性](./06-error-handling-security.md)

## 综合练习

[阶段 5 综合练习：用户管理系统](./practice.md)

---

> 完成阶段 4 后进入。
